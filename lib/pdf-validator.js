...
{"vendor": null}`;
    ...
    async processPdf(pdfBuffer, filename) {
        ...
        return {
            ...
            emailFound: contactPerson.emailFound !== false, // Default to true if not specified
...
Return ONLY valid JSON in this exact format:
{
  ...
  "reason": "explanation of what was found and why it passed/failed"
    ...
    determineContactPerson(vendorName, poValidation, dateValidation, rateValidation) {
        ...
        // Get contact names from vendor data
        const mainContactName = vendorInfo.mainContact;
        const fumName = vendorInfo.fum;
        
        // Get emails from email contacts JSON by contact name
        const mainContactEmail = mainContactName && mainContactName.toString().toLowerCase() !== 'nan' 
            ? emailContacts.getEmail(mainContactName.toString().trim()) : null;
        const fumEmail = fumName && fumName.toString().toLowerCase() !== 'nan'
            ? emailContacts.getEmail(fumName.toString().trim()) : null;
        ...
        if (!poFailed && !dateFailed && !rateFailed && !isVariableRate) {
            if (mainContactEmail) {
                ...
                // Main Contact email not found - will need manual handling
                return {
                    name: mainContactName || 'Main Contact',
                    email: null,
                    emailFound: false,
                    role: "Main Contact",
                    reason: "All validations passed but Main Contact email not found - requires manual handling"
                };
            }
        }
        ...
        if (fumEmail) {
            ...
            // FUM email not found - will need manual handling
            const reasons = [];
            if (poFailed) reasons.push("PO validation failed");
            if (dateFailed) reasons.push("date validation failed");
            if (rateFailed) reasons.push("rate validation failed");
            ...
            return {
                ...
                emailFound: false,
                role: "FUM",
                reason: `Issue requires FUM attention (${reasons.join(", ")}) but FUM email not found - requires manual handling`
            };
        }
