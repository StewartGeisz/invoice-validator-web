...
import emailContacts from './email-contacts.js';
...
class ApprovalHandler {
    ...
    /**
     * Get Main Contact email from email contacts database by contact name
     */
    _getMainContactEmail(contactName) {
        if (!contactName) {
            return null;
        }
        
        const email = emailContacts.getEmail(contactName.toString().trim());
        if (!email) {
            console.log(`   ⚠️  No email found for contact name: ${contactName}`);
        }
        return email;
    }
    ...
    async processApprovalReply(email) {
        ...
        // Get original sender from multiple possible locations (for backward compatibility)
        const initialSender = auditEntry.originalSenderEmail || 
                             auditEntry.metadata?.originalSenderEmail || 
                             auditEntry.metadata?.sourceEmail || 
                             null;
    ...
    async _handleFUMApproval(vendorName, filename, referenceId, initialSenderEmail, fumEmail, auditEntry) {
        ...
        // Get Main Contact name from vendor data
        const mainContactName = this.vendorData[vendorName]?.mainContact || null;
        
        if (!mainContactName || mainContactName.toString().toLowerCase() === 'nan') {
            console.log(`   ⚠️  No Main Contact name found for vendor: ${vendorName}`);
            return await this._handleMissingMainContactEmail(vendorName, filename, referenceId, initialSenderEmail, auditEntry);
        }

        // Get Main Contact email from email contacts database by contact name
        const mainContactEmail = this._getMainContactEmail(mainContactName);
        if (!mainContactEmail) {
            console.log(`   ⚠️  No Main Contact email found for contact: ${mainContactName}`);
            // Send notification to original sender that manual handling is required
            return await this._handleMissingMainContactEmail(vendorName, filename, referenceId, initialSenderEmail, auditEntry);
        }
        ...
        if (initialSenderEmail) {
            const senderEmail = initialSenderEmail;
            const senderHtml = `
                <!DOCTYPE html>
                <html>
                <head>
    ...
    /**
     * Handle case where Main Contact email is not found after FUM approval
     */
    async _handleMissingMainContactEmail(vendorName, filename, referenceId, originalSenderEmail, auditEntry) {
        console.log(`   ⚠️  Main Contact email not found - notifying original sender for manual handling`);

        if (!originalSenderEmail) {
            console.log(`   ⚠️  No original sender email found`);
            return { processed: false, reason: 'No original sender found' };
        }

        // IMPORTANT: Original sender should NOT be redirected in dev mode - always use actual sender email
        const senderEmail = originalSenderEmail;
        ...
        const manualHandlingHtml = `
            ...
            <body>
                <div class="header">
                    <h2>Manual Handling Required - FUM Approved</h2>
                </div>
                <div class="content">
                    <p>The invoice for <strong>${vendorName}</strong> was <strong>approved by FUM</strong>, but <strong>no Main Contact email address was found</strong>.</p>
                    
                    <div class="warning-box">
                        <p><strong>⚠️ Action Required:</strong></p>
                        <p>Please manually contact the Main Contact for this vendor to complete the approval process.</p>
                        <p><strong>Vendor:</strong> ${vendorName}</p>
                        <p><strong>Invoice File:</strong> ${filename}</p>
                        <p><strong>Reference ID:</strong> ${referenceId}</p>
                    </div>
                    ...
                    <p><strong>Status:</strong> ✅ Approved by FUM - Ready for Main Contact approval</p>
        ...
        try {
            await emailService.sendEmail({
                to: senderEmail,
                subject: `Manual Handling Required: FUM Approved - ${vendorName} - ${filename} [Ref: ${referenceId}]`,
                html: manualHandlingHtml
            });
            console.log(`   ✅ Sent manual handling notification to original sender: ${senderEmail}`);
            return { processed: true, decision: 'approved', manualHandling: true, notified: senderEmail };
        } catch (error) {
            console.error(`   ❌ Failed to send manual handling notification:`, error.message);
            return { processed: false, reason: 'Failed to send manual handling notification' };
        }
    }
}
