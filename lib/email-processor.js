...
class EmailProcessor {
    ...
    _renderTemplate(template, variables) {
        ...
        rendered = rendered.replace(/{{#if\s+(\w+)}}([\s\S]*?){{\/if}}/g, (match, condition, content) => {
            ...
            // Handle boolean values and string booleans ('true'/'false')
            ...
            // For other truthy values, show content
    ...
    /**
     * Send manual handling notification when contact email is not found
     */
    async _sendManualHandlingNotification(validationResult, filename, pdfBuffer, originalSenderEmail, referenceId) {
        const contactName = validationResult.contact_person || 'Contact';
        const contactRole = validationResult.contact_role || 'Contact';
        
        const manualHandlingHtml = `
            <!DOCTYPE html>
            <html>
            <head>
                <meta charset="UTF-8">
                <style>
                    ...
                    .warning-box { background-color: #fff3cd; padding: 15px; border-left: 4px solid #ff9800; margin: 15px 0; border-radius: 3px; }
                    .validation-details { background-color: white; padding: 15px; border-radius: 5px; margin: 15px 0; border-left: 4px solid #2196F3; }
                    .detail-row { margin: 10px 0; padding: 8px; background-color: #f5f5f5; border-radius: 3px; }
            ...
            <body>
                <div class="header">
                    <h2>Manual Handling Required</h2>
                </div>
                <div class="content">
                    <p>An invoice has been processed and validated, but <strong>no email address was found</strong> for the required contact person.</p>
                    
                    <div class="warning-box">
                        <p><strong>⚠️ Action Required:</strong></p>
                        <p>The invoice for <strong>${validationResult.vendor}</strong> requires manual handling because:</p>
                        <ul>
                            <li><strong>Required Contact:</strong> ${contactName} (${contactRole})</li>
                            <li><strong>Issue:</strong> No email address found in contact database</li>
                        </ul>
                        <p>Please manually contact ${contactName} (${contactRole}) to process this invoice.</p>
                    </div>
                    ...
                    <div class="validation-details">
                        ...
                        <div class="detail-row">
                            ...
                            ${this._formatStatus(validationResult.po_valid)} - ${validationResult.po_reason || 'N/A'}
                        </div>
                        <div class="detail-row">
                            <span class="detail-label">Date Range:</span> 
                            ${this._formatStatus(validationResult.date_valid)} - ${validationResult.date_reason || 'N/A'}
                        </div>
                        <div class="detail-row">
                            <span class="detail-label">Rate Amount:</span> 
                            ${this._formatStatus(validationResult.rate_valid)} - ${validationResult.rate_reason || 'N/A'}
                        </div>
                    </div>
                    ...
                    <p>The validated invoice PDF is attached to this email.</p>
        ...
        try {
            await emailService.sendEmail({
                ...
                subject: `Manual Handling Required: ${validationResult.vendor} - ${filename} [Ref: ${referenceId}]`,
                html: manualHandlingHtml,
                attachments: [{
                    filename: filename,
                    content: pdfBuffer,
            ...
            console.log(`      ✅ Manual handling notification sent to original sender: ${originalSenderEmail}`);
        } catch (emailError) {
            console.error(`      ❌ Failed to send manual handling notification:`, emailError.message);
        }
    }
    ...
    async processEmail(email) {
        ...
        for (let i = 0; i < pdfAttachments.length; i++) {
            ...
            try {
                ...
                // Get email sender (the person who forwarded the invoice) - THIS IS THE ORIGINAL SENDER
                const emailSender = this._extractEmailAddress(email.from);
                // IMPORTANT: Original sender should NOT be redirected in dev mode - always use actual sender email
                const originalSenderEmail = emailSender || null;
                
                // Store original sender email in validation result for audit tracking
                validationResult.originalSenderEmail = emailSender; // Store actual email before environment processing
                
                // Get contact email from validation result
                const contactEmail = validationResult.contact_email;
                const emailFound = validationResult.emailFound !== false; // Default to true if not specified
                
                // If email not found, send notification to original sender instead
                if (!contactEmail || !emailFound) {
                    console.log(`      ⚠️  No contact email found for ${validationResult.contact_person} (${validationResult.contact_role})`);
                    console.log(`      📧 Sending manual handling notification to original sender: ${originalSenderEmail}`);
                    
                    await this._sendManualHandlingNotification(
                        validationResult,
                        filename,
                        pdfBuffer,
                        originalSenderEmail,
                ...
                // isFailure should only be true for FUM (when validation fails)
                // Main Contact should NEVER see the "Action Required" approval instructions
                ...
                const contactVars = {
                    ...
                    filename: filename,
                    hasAttachment: 'true',
                    isFailure: isFailure, // Pass boolean directly - template renderer handles it
                    referenceId: referenceId
                };
                ...
                // Send summary email to the original sender (person who forwarded the invoice)
                console.log(`      📧 Sending summary to original sender: ${originalSenderEmail}...`);
                try {
                    await emailService.sendEmail({
                        to: originalSenderEmail,
                    ...
                    console.log(`      ✅ Summary sent to original sender: ${originalSenderEmail}`);
                } catch (emailError) {
                ...
                // Send error notification to the original sender (person who forwarded the invoice)
                const emailSender = this._extractEmailAddress(email.from);
                // IMPORTANT: Original sender should NOT be redirected in dev mode - always use actual sender email
                const originalSenderEmail = emailSender || null;
                
                console.log(`      📧 Sending error notification to original sender: ${originalSenderEmail}...`);
                try {
                    await emailService.sendEmail({
                        to: originalSenderEmail,
                    ...
                    console.log(`      ✅ Error notification sent to original sender: ${originalSenderEmail}`);
                } catch (emailError) {
